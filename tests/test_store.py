...
class TestSentryStore:
    ...
    def test_add_chunks_preserves_extra_metadata(self, tmp_store):
        ...
        stored = tmp_store.collection.get(include=["metadatas"])
        assert stored["metadatas"][0]["segment_index"] == 1
        assert stored["metadatas"][0]["segmentation"] == "shot"
...
class TestStoreBackend:
    ...
    def test_shot_backend_collection_name(self, tmp_path):
        from sentrysearch.store import SentryStore

        store = SentryStore(db_path=tmp_path / "db", backend="gemini", segmentation="shot")
        assert store.collection.name == "dashcam_shots"

    def test_shot_local_model_collection_name(self, tmp_path):
        from sentrysearch.store import SentryStore
        ...
        assert store.collection.name == "dashcam_shots_local_qwen2b"
    ...
    def test_doubao_backend_collection_name(self, tmp_path):
        from sentrysearch.store import SentryStore

        store = SentryStore(db_path=tmp_path / "db", backend="doubao")
        assert store.collection.name == "dashcam_chunks_doubao"

    def test_qwen_backend_collection_name(self, tmp_path):
        from sentrysearch.store import SentryStore

        store = SentryStore(db_path=tmp_path / "db", backend="qwen")
        assert store.collection.name == "dashcam_chunks_qwen"
    ...
    def test_get_segmentation_defaults_to_chunk(self, tmp_store):
        assert tmp_store.get_segmentation() == "chunk"

    def test_get_segmentation_shot(self, tmp_path):
        from sentrysearch.store import SentryStore

        store = SentryStore(db_path=tmp_path / "db", backend="gemini", segmentation="shot")
        assert store.get_segmentation() == "shot"
...
class TestDetectBackend:
    ...
    def test_detects_doubao(self, tmp_path):
        from sentrysearch.store import SentryStore, detect_backend

        store = SentryStore(db_path=tmp_path / "db", backend="doubao")
        store.add_chunk("c1", _make_embedding(dim=2048), {
            "source_file": "v.mp4", "start_time": 0.0, "end_time": 30.0,
        })
        assert detect_backend(tmp_path / "db") == "doubao"

    def test_detects_qwen(self, tmp_path):
        from sentrysearch.store import SentryStore, detect_backend

        store = SentryStore(db_path=tmp_path / "db", backend="qwen")
        store.add_chunk("c1", _make_embedding(dim=1024), {
            "source_file": "v.mp4", "start_time": 0.0, "end_time": 30.0,
        })
        assert detect_backend(tmp_path / "db") == "qwen"
...
class TestDetectIndex:
    ...
    def test_detects_doubao_model(self, tmp_path):
        from sentrysearch.store import SentryStore, detect_index
        ...
        assert detect_index(tmp_path / "db") == ("doubao", "doubao-embedding-vision-251215")

    def test_detects_qwen_model(self, tmp_path):
        from sentrysearch.store import SentryStore, detect_index
        ...
        assert detect_index(tmp_path / "db") == ("qwen", "qwen3-vl-embedding")
    ...
    def test_model_specific_local_preferred_over_legacy_local(self, tmp_path):
        from sentrysearch.store import SentryStore, detect_index

        db = tmp_path / "db"
        legacy = SentryStore(db_path=db, backend="local")
        legacy.add_chunk("legacy", _make_embedding(), {
            "source_file": "legacy.mp4", "start_time": 0.0, "end_time": 30.0,
        })
        model_specific = SentryStore(db_path=db, backend="local", model="qwen2b")
        model_specific.add_chunk("modern", _make_embedding(), {
            "source_file": "modern.mp4", "start_time": 0.0, "end_time": 30.0,
        })
        assert detect_index(db) == ("local", "qwen2b")


class TestDetectIndexDetails:
    def test_detects_shot_segmentation(self, tmp_path):
        from sentrysearch.store import SentryStore, detect_index_details

        store = SentryStore(db_path=tmp_path / "db", backend="gemini", segmentation="shot")
        store.add_chunk("c1", _make_embedding(), {
            "source_file": "v.mp4", "start_time": 0.0, "end_time": 5.0,
        })
        assert detect_index_details(tmp_path / "db") == ("gemini", None, "shot")

    def test_prefers_chunk_over_shot(self, tmp_path):
        from sentrysearch.store import SentryStore, detect_index_details

        chunk_store = SentryStore(db_path=tmp_path / "db", backend="gemini", segmentation="chunk")
        chunk_store.add_chunk("chunk1", _make_embedding(), {
            "source_file": "v.mp4", "start_time": 0.0, "end_time": 30.0,
        })
        shot_store = SentryStore(db_path=tmp_path / "db", backend="gemini", segmentation="shot")
        shot_store.add_chunk("shot1", _make_embedding(), {
            "source_file": "v.mp4", "start_time": 0.0, "end_time": 5.0,
        })
        assert detect_index_details(tmp_path / "db") == ("gemini", None, "chunk")

    def test_filters_by_segmentation(self, tmp_path):
        from sentrysearch.store import SentryStore, detect_index_details

        shot_store = SentryStore(db_path=tmp_path / "db", backend="local", model="qwen2b", segmentation="shot")
        shot_store.add_chunk("shot1", _make_embedding(), {
            "source_file": "v.mp4", "start_time": 0.0, "end_time": 5.0,
        })
        assert detect_index_details(tmp_path / "db", segmentation="shot") == ("local", "qwen2b", "shot")
