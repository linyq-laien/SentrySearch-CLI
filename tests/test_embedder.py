...
class TestEmbedderFactory:
    ...
    def test_get_embedder_doubao_backend(self):
        with patch("sentrysearch.doubao_embedder.DoubaoEmbedder") as MockDoubao:
            mock_instance = MagicMock()
            MockDoubao.return_value = mock_instance
            reset_embedder()
            result = get_embedder("doubao")
            MockDoubao.assert_called_once_with()
            assert result is mock_instance

    def test_get_embedder_qwen_backend(self):
        with patch("sentrysearch.qwen_embedder.QwenEmbedder") as MockQwen:
            mock_instance = MagicMock()
            MockQwen.return_value = mock_instance
            reset_embedder()
            result = get_embedder("qwen")
            MockQwen.assert_called_once_with()
            assert result is mock_instance
